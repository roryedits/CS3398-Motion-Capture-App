...
void setup() {
  // Setup graphics
  size(1200, 700, P3D);
  s = loadShape("glock.obj");
  head_obj = loadShape("head.obj");
  torso_obj = loadShape("torso.obj");
 ...
 // head_obj.rotate(PI);
  ...
  torso_obj.vertex(0,0);
  torso_obj.scale(19.0);
  ...
  head = loadStrings("/Users/Heyseb1/Documents/D2/processing_data/$cenario7/Scenario7_Head.csv");
...
void draw() {
  ...
  //make this into a function called getValue, increment a count everytime it enters, only let it go as long as the length of the count
  //return the yaw pitch and roll 
  getValue();
  ...
  // Draw board
  pushMatrix();
  translate(width-50, height/2.0, -500);
  ...
  // Draw board
  pushMatrix();
  translate(width/2.0, -10, -500);
  
  //rotateY(radians(100));
  //rotateX(radians(pitch));
  rotateZ(radians(180)); // add 180 to get upright head
  shape(head_obj);
  popMatrix();
  
    // Draw board
  pushMatrix();
  translate((width/2.0) - 25, height-175, -500);
